...
CREATE TABLE IF NOT EXISTS conversation_turn_executions (
...
CREATE INDEX IF NOT EXISTS idx_conversation_turn_executions_conversation_state
ON conversation_turn_executions (conversation_id, state, id);

CREATE UNIQUE INDEX IF NOT EXISTS uq_conversation_turn_executions_active_conversation
ON conversation_turn_executions (conversation_id)
WHERE state NOT IN ('delivered', 'aborted');

CREATE INDEX IF NOT EXISTS idx_conversation_turn_executions_reconciliation
