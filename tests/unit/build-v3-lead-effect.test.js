...
describe('Build V3 Lead Effect — the v3 decision reaches the lead contract', () => {
  ...
  test('keeps service scope separate from fulfillment in new v3 decisions', () => {
    const lead = buildV3LeadEffect(authorizedTurn({
      v3_decision: decision([
        mutation('product', 'Bloques de Hormigón'),
        mutation('commune', 'Rancagua'),
        mutation('quantity', 120),
        mutation('service_scope', 'material'),
        mutation('fulfillment', 'delivery'),
      ]),
    }));

    expect(lead.service).toBe('despacho');
    expect(lead.qualification_context).toMatchObject({
      service_scope: 'material',
      fulfillment: 'delivery',
    });
  });

  test('keeps reading historical modality values', () => {
    const serviceFor = (modality) => buildV3LeadEffect(authorizedTurn({
      qualification_context: {
        product: 'Bloques de Hormigón', commune: 'Santiago', quantity: 20, modality,
      },
      v3_decision: decision([]),
    })).service;
  ...
  test('prioritizes installation, then fulfillment, then material for the legacy lead service', () => {
    const serviceFor = ({ serviceScope, fulfillment }) => buildV3LeadEffect(authorizedTurn({
      v3_decision: decision([
        mutation('product', 'Bloques de Hormigón'),
        mutation('commune', 'Rancagua'),
        mutation('quantity', 120),
        mutation('service_scope', serviceScope),
        ...(fulfillment ? [mutation('fulfillment', fulfillment)] : []),
      ]),
    })).service;

    expect(serviceFor({ serviceScope: 'installation', fulfillment: 'delivery' })).toBe('instalacion');
    expect(serviceFor({ serviceScope: 'both', fulfillment: 'delivery' })).toBe('material e instalacion');
    expect(serviceFor({ serviceScope: 'material', fulfillment: 'delivery' })).toBe('despacho');
    expect(serviceFor({ serviceScope: 'material', fulfillment: 'pickup' })).toBe('retiro');
    expect(serviceFor({ serviceScope: 'material' })).toBe('material');
  });
  ...
  test('maps persisted v3 fact names when the effect turn only adds product and quantity', () => {
    const lead = buildV3LeadEffect(authorizedTurn({
      qualification_context: {
        commune: 'Santiago',
      ...
      v3_decision: decision([
        mutation('product', 'Bloques de Hormigón'),
        mutation('quantity', 100),
      ]),
    }));

    expect(lead.service).toBe('instalacion');
    expect(lead.city).toBe('Santiago');
    expect(lead.requirement).toContain('Bloques de Hormigón');
    expect(lead.requirement).toContain('100');
    expect(lead.requirement).not.toContain('[object Object]');
    expect(lead.qualification_context).toMatchObject({
  ...
  test('composes the requirement from persisted v3 facts on a later confirmation turn', () => {
    const lead = buildV3LeadEffect(authorizedTurn({
      qualification_context: {
        product: 'Bloques de Hormigón',
        quantity: 100,
    ...
    expect(lead).toMatchObject({
      service: 'instalacion',
      city: 'Santiago',
      requirement: 'Bloques de Hormigón 100',
    });
  });
