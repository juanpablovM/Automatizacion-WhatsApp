-- Preserve bot sender classification and final human authority; acquire exact v3 reserved outbox rows.
...
BEGIN
  ...
  ELSIF v_message.dispatch_phase = 'reserved' THEN
    v_token := md5(random()::text || clock_timestamp()::text || p_idempotency_key);
    UPDATE messages m SET dispatch_phase = 'claimed', dispatch_token = v_token,
      claimed_at = NOW(), reconciliation_required = FALSE,
