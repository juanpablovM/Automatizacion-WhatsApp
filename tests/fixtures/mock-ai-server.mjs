...
export const buildValidProposal = (policy, plan) => {
  ...
  const productObservation = observations.find(({ concept }) => concept === 'product');
  const plannedCatalogResolution = plan?.catalog_resolution;
  const catalog_resolution = plannedCatalogResolution
    ? {
        status: plannedCatalogResolution.status,
        evidence_quote: plannedCatalogResolution.evidence_quote ?? null,
        evidence_occurrence: plannedCatalogResolution.evidence_occurrence ?? null,
        grounding_ref: plannedCatalogResolution.grounding_ref ?? null,
      }
    : productObservation
      ? {
          status: 'matched',
          evidence_quote: productObservation.evidence_quote,
          evidence_occurrence: productObservation.evidence_occurrence,
          grounding_ref: productObservation.grounding_ref,
        }
      : {
  ...
  return {
    ...
    primary_request: plan?.primary_request ?? null,
    catalog_resolution,
    observations,
    state_mutations,
    effect_requests,
