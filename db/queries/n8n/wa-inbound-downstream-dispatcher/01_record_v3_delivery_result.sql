...
WITH target AS MATERIALIZED (
  ...
  JOIN messages message ON message.id = $2::BIGINT
  WHERE execution.decision_id = $1::TEXT
    AND (execution.delivery_message_id = $2::BIGINT
      OR (execution.state = 'aborted' AND execution.delivery_receipt_ref->>'delivery_message_id' = $2::TEXT))
    AND execution.delivery_key = $5::TEXT
    AND execution.state IN ('delivery_pending', 'delivered', 'reconciliation_required', 'aborted', 'aborted')
    AND message.direction = 'outgoing'
    AND message.idempotency_key = execution.delivery_key
    AND message.delivery_status = $3::TEXT
    AND $3::TEXT IN ('sent', 'failed', 'unknown', 'cancelled')
    ...
    AND (
      ...
      OR ($3::TEXT IN ('failed', 'unknown', 'cancelled'))
    )
  ...
  SET state = CASE
        WHEN target.delivery_status = 'cancelled' THEN 'aborted'
        WHEN target.delivery_status = 'sent' THEN 'delivered'
        ELSE 'reconciliation_required'
      END,
      delivery_message_id = CASE WHEN target.delivery_status = 'cancelled' THEN NULL ELSE execution.delivery_message_id END,
  ...
  SELECT 'v3_delivery_recorded', 'conversation_turn_execution', transitioned.id,
         ...
         CASE WHEN transitioned.state = 'aborted' THEN 'human_control_cancelled'
              WHEN transitioned.state = 'delivered' THEN 'success' ELSE 'reconciliation_required' END,
         jsonb_build_object(
  ...
  CROSS JOIN receipt
  WHERE execution.state IN ('delivered', 'reconciliation_required', 'aborted')
