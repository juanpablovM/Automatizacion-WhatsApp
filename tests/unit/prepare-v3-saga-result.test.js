...
describe('Prepare V3 Saga Result rejoins the saga with the turn context', () => {
  ...
  test('returns committed qualification evidence before a lead is created', () => {
    const committed = { product: 'Pastelones', commune: 'Santiago', quantity: '20 unidades' };
    const [out] = runCodeNode([{ json: {
      state: 'delivery_pending', v3_persisted_qualification_context: committed,
    } }], { ...turnContext, qualification_context: { product: 'Pastelones' } });
    expect(out.json.qualification_context).toEqual(committed);
    expect(JSON.parse(out.json.qualification_context_json)).toEqual(committed);
    expect(out.json.should_create_lead).toBe(false);
  });
  ...
  test('propagates the persisted waiting state for a v3 primary request', () => {
    const [out] = runCodeNode([{ json: {
      decision_id: 'decision-v3-waiting',
      conversation_status_code: 'waiting_user',
      current_step: 'name',
      pending_question_key: 'name',
    } }], {
      ...turnContext,
  ...
  test('prefers the post-commit suffixed state over the pre-commit merge state', () => {
    const [out] = runCodeNode([{ json: {
      decision_id: 'decision-v3-merged-waiting',
      conversation_status_code: 'active',
      conversation_status_code_1: 'waiting_user',
      current_step: 'confirm',
      current_step_1: 'fulfillment',
      pending_question_key: null,
      pending_question_key_1: 'fulfillment',
      v3_persisted_conversation_status_code: 'waiting_user',
      v3_persisted_current_step: 'fulfillment',
      v3_persisted_pending_question_key: 'fulfillment',
    } }], turnContext);

    expect(out.json).toMatchObject({
      conversation_status_code: 'waiting_user',
      current_step: 'fulfillment',
      pending_question_key: 'fulfillment',
    });
  });

  test('resumes the existing ClickUp and notification pipeline only from a receipted v3 lead', () => {
    const [out] = runCodeNode([{ json: {
      decision_id: 'decision-v3-lead',
      lead_id: 112,
      service: 'instalacion',
      city: 'Santiago',
  ...
  test('does not resume the lead pipeline from a failed create_lead receipt', () => {
    const [out] = runCodeNode([{ json: {
      decision_id: 'decision-v3-lead-failed',
      lead_id: 112,
      effect_receipt_refs: [{
        version: 'v3_effect_receipt/v1',
        effect_type: 'create_lead',
